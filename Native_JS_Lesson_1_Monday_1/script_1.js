...
U (Update) - это значит что у кого-то из элементов массива произошли изменения. Например поменялся параметр name
             после каждого изменения мы будем его регистрировать
             Когда мы хотим сделать update (например изменить свойство name) у объекта в массиве.
             Мы создадим новый массив. Тот элемент в котором изменения не произошли добавим в новый массив без
             изменений. А тот элемент где изменения были добавим в новый массив уже с изменениями

             let oldArray = [X,X]
             let newArray = [XY,X]   map map map

D (Delete) - 1 Если мы хотим что-либо удалить то делаем копию массива из которого хотим удалить элемент
             Чтобы реакт понял что произошли изменения которые надо отрисовать
             2 Оставляем только те элемент из старого массива которые нам нужны, а те которые не нужны
             в новом массиве, не берем их в новый массив и оставляем в старом

             let oldArray = [X,X]
             let newArray = [X, ]             filter filter filter

Если у нас сложный объект с большой вложенностью других объектов, то полную глубокую копию мы не делаем
потому что чаще нам нужно достучаться до конкретного объекта в глубине структуры без всей структуры


             CRUD операции, примеры:

// Create
const addStudent = [] - создаем новый массив

внутрь нового массива кладем содержимое students массива (перекладываем все ссылки которые есть в students)

const addStudent = [...students]

А потом ставим запятую и добавляем новый литерал объекта

const addStudent = [...students, {name: 'anna', age:28}]

// Update

Увеличиваем одному из студентов возраст

const updateBobAge = students.map() - обращаемся ко всем элементам массива students поочереди
это не будет являться сплошным копированием это будет являться выборочным копированием

const updateBobAge = students.map(s => s.name === 'Bob') - мы будем изменять только тот объект в массиве у которого
name 'Bob' а остальный трогать не будет поэтому это не будет являться сплошным копированием

const updateBobAge = students.map(s => s.name === 'Bob' ? ) - если мы хотим внести изменения в Bob
то сначала нужно сделать копию объекта Bob поэтому

const updateBobAge = students.map(s => s.name === 'Bob' ? {...s})

а затем сказать что мы перезапишем свойство age которое будет равно уже 35

const updateBobAge = students.map(s => s.name === 'Bob' ? {...s, age: 35})

а если студеент не Bob то мы в новый массив положим старую ссылку потому что в нем ничего не поменялось
то есть добавим в новый литерал ссылку на старыйобъект без изменений
а там где мы меняли свойство мы указали новый литерал объекта {...s, age: 35}
а там где не меняли новый литерал объекта не создавали : s )

Таким образом получим новый массив в котором какие-то элементы будут равны элементам из старого массива
потому что не изменялись, а какие-то не будут равны потому что создавались с помощью нового литерала

// Delete

Удаляем одного из студентов
Говорим что будем брать всех студентов кроме Bob

const removeStudent = students.filter( s => s.name !== 'Bob')   - опять не надо делать глубоких копий
filter так же возвращает новый массив что будет заставлять реакт перерисовываться

Если нигде больше нет ссылок на объект который удаляется, то он удалится из памяти

                                Усложняем структуру объекта
...
Задача изменить Василий на Mathem

1 Нужно перебрать массив с помощью метода map

const newStudents = students.map(s => )

Далее мы вносим изменения в объект с name 'Bob'
и так как в этом объекте будут происходить изменения поэтому мы должны сделать новый массив
а так как в другом элементе изменения не происходят то его мы будем добавлять без литерала объекта

const newStudents = students.map(s => s.name === 'Bob' ? {} : s )

Делаем копию объекта с имненем Bob {...s} а затем укажем что именно мы будем менять
а менять мы будем свойство объекта friends которое является массивом, и так как оно будет меняться
то для него нужно создавать новый литерал объекта, то есть новый массив, а новый массив нам возвращает метод map
поэтому используем map в map

const newStudents = students.map(s => s.name === 'Bob' ? {...s, friends: } : s )

Для того чтобы еще раз обратиться к методу map возьмем массив который был а он у нас в s.friends
и к нему применим map внутри него будем брать всех друзей поочереди f
и возьмем того у которого name равно Василий

const newStudents = students.map(s => s.name === 'Bob' ? {...s, friends:
        s.friends.map( f => f.name === 'Василий')} : s )

И если я нашел такого студента (если true) то делаем копию этого объекта f

const newStudents = students.map(s => s.name === 'Bob' ? {...s, friends:
        s.friends.map( f => f.name === 'Василий'
            ? {...f})} : s )

И далее указываю параметр который хочу изменить name на Mathew и изменяю

const newStudents = students.map(s => s.name === 'Bob' ? {...s, friends:
        s.friends.map( f => f.name === 'Василий'
            ? {...f, name: 'Mathew'})} : s )

А если это не тот friend который не Василий, то ничего с ним не делаю f

const newStudents = students.map(s => s.name === 'Bob'
    ? {...s, friends:
        s.friends.map( f => f.name === 'Василий'
            ? {...f, name: 'Mathew'}
            : f
...
Стремимся чтобы структура объектов была не более 3 - х уровней вложенности
*/
...
// Update
const updateBobAge = students.map(s => s.name === 'Bob' ? {...s, age: 35} : s )

// Delete
const removeStudent = students.filter( s => s.name !== 'Bob')
...
/*
Пример не правильного массива
const addStudent = [...students, {name: 'anna', age:28}] - правильно
const wrongAddStudent = students.push({name: 'anna', age:28}]) - не правильно

Потому что мы добавляем элемент в массив без создания нового литерала объекта, а значит
что реакт увидит старую обертку и не станет отрисовывать новые изменения
*/

const wrongAddStudent = students.push()